...
interface Collection {
  ...
  symbol: string;
  imageHash: string;
  contractAddress: string; // Added to store the contract address
}
...
export default function Dashboard(): JSX.Element {
  ...
  const createCollection = async () => {
    ...
    try {
      ...
      // Add the ERC20 token address from .env or state (ensure it's correctly set in the frontend)
      const tokenAddress = process.env.REACT_APP_ERC20_TOKEN_ADDRESS;

      if (!tokenAddress) {
        console.error('ERC20 token address is not defined.');
        return;
      }

      // Modify the method call to include the token address
      const tx = await marketplace.methods.createCollection(collectionName, symbol, imageIpfsHash, tokenAddress).send({ from: account });
...
const deployContract = async () => {
  ...
  try {
    ...
    const deployTx = factory.deploy({
      data: NFTCollectionABI.bytecode,
      arguments: [collectionName, symbol, account, tokenAddress], // Updated to include tokenAddress
    }).encodeABI();
...
const mintNFT = async () => {
  ...
  // Set price to the minimum value in the ERC20 token
  const price = '1'; // 1 wei
    ...
    try {
      ...
      if (Array.isArray(collectionsList)) {
        setCollections(collectionsList.map((collection: any) => ({
          ...
          contractAddress: collection.contractAddress || collection[4], // Added to store the contract address
        }))); // Spread the new collections to trigger re-render
      } else {
        setCollections([]);
      }
    } catch (error: any) {
  ...
  const handleBuy = async (collection: Collection) => {
    console.log(`Attempting to buy NFT from collection: ${collection.name}`);
    ...
    const nftCollectionContract = new web3.eth.Contract(NFTCollectionABI.abi, collection.contractAddress);
  
    try {
      const tokenId = 1; // Adjust this according to your logic
      const nftData = await nftCollectionContract.methods.nfts(tokenId).call();
  
      if (nftData && typeof nftData === 'object' && 'price' in nftData) {
        const price = '1'; // Set to 1 wei, the smallest possible amount
  
        // Step 1: Get the ERC20 token contract
        const erc20TokenAddress = process.env.REACT_APP_ERC20_TOKEN_ADDRESS;
        const erc20TokenContract = new web3.eth.Contract(ERC20ABI.abi, erc20TokenAddress);
  
        // Step 2: Approve the NFT contract to spend tokens
        const approvalTx = await erc20TokenContract.methods.approve(collection.contractAddress, price).send({ from: account });
        ...
        // Refresh the collections after purchase
        fetchCollections();
      } else {
        console.error("NFT data is not in the expected format or does not contain a price.");
      }
    } catch (error: any) {
